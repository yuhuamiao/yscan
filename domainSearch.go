...
//			URLFormat: "https://www.sogou.com/web?query=site:%s",
//			Enabled:   true,
//		},
//	}
//
...
//			log.Printf("[%s] %s status=%d len=%d", s.SourceName, r.Request.URL, r.StatusCode, len(r.Body))
//		})
//
//		// 并发查询
...
//			return nil
//		}
//		// 使用自定义 resolver + 过滤保留网段
//		ips := resolveA(context.Background(), subdomain)
//		return ips
//	}
package main
...
// 仅启用 Bing；如需 Baidu 改 Enabled=true 并提供 Cookie
var defaultSearchEngines = []SearchEngineConfig{
	{
		Name:      "bing",
		URLFormat: "https://www.bing.com/search?q=site:%s&count=50",
...
func (s *SearchEngineCollector) Collect(domain string, timeout time.Duration) ([]CollectResult, error) {
	...
	// 可选代理
	// c.SetProxy("http://user:pass@proxy:port")

	// 可选 Cookie（Baidu 验证码时填写）
	baiduCookie := ""
	bingCookie := ""
	c.OnRequest(func(r *colly.Request) {
		r.Headers.Set("Accept-Language", "zh-CN,zh;q=0.9,en;q=0.8")
		if strings.Contains(r.URL.Host, "baidu.com") && baiduCookie != "" {
			r.Headers.Set("Cookie", baiduCookie)
		}
		if strings.Contains(r.URL.Host, "bing.com") && bingCookie != "" {
			r.Headers.Set("Cookie", bingCookie)
		}
	})
	...
	c.OnHTML("ol#b_results li.b_algo a", func(e *colly.HTMLElement) {
		...
		extractAndAppend(text, re, domain, &results, &resultMutex, s)
	})
	...
	// c.OnResponse(func(r *colly.Response) {
	// 	head := string(r.Body)
	// 	if len(head) > 400 { head = head[:400] }
	// 	log.Printf("[%s] %s status=%d len=%d head=%.200s", s.SourceName, r.Request.URL, r.StatusCode, len(r.Body), head)
	// })
...
func extractAndAppend(text string, re *regexp.Regexp, domain string, results *[]CollectResult, mu *sync.Mutex, s *SearchEngineCollector) {
	if matches := re.FindAllString(text, -1); len(matches) > 0 {
		mu.Lock()
		defer mu.Unlock()
		for _, raw := range matches {
			...
			if !s.containsSubdomain(*results, sub) {
				*results = append(*results, s.createResult(sub, "search_engine"))
			}
		}
	}
}
